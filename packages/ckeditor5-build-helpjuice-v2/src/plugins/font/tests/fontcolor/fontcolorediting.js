...
describe('FontColorEditing', () => {
	...
	it('should set proper schema rules', () => {
		expect(
			editor.model.schema.checkAttribute(['$block', '$text'], 'fontColor')
		).to.be.true;
		expect(
		...
		expect(editor.model.schema.checkAttribute(['$block'], 'fontColor')).to
			.be.false;
	});

	it('has the attribute marked with the isFormatting property', () => {
		expect(
			editor.model.schema.getAttributeProperties('fontColor')
		).to.include({
			isFormatting: true,
		});
	});
		...
		describe('convert different color version', () => {
			...
			tests.forEach((test) => {
				it(`should convert fontColor attribute: "${test}" to proper style value.`, () => {
					setModelData(
						doc,
						`<paragraph>fo<$text fontColor="${test}">o b</$text>ar</paragraph>`
					);

					expect(editor.getData()).to.equal(
						`<p>fo<span style="color:${test};">o b</span>ar</p>`
					);
				});
			});
		});
	...
	describe('data pipeline conversions', () => {
		...
		it('should convert from element with defined style when with other styles', () => {
			const data =
				'<p>f<span style="font-size: 18px;color: rgb(10, 20, 30);">o</span>o</p>';

			editor.setData(data);

			expect(getModelData(doc)).to.equal(
				'<paragraph>[]f<$text fontColor="rgb(10,20,30)">o</$text>o</paragraph>'
			);
			expect(editor.getData()).to.equal(
				'<p>f<span style="color:rgb(10,20,30);">o</span>o</p>'
			);
		});
		...
		describe('should convert from different color versions', () => {
			...
			tests.forEach((test) => {
				it(`should convert fontColor attribute: "${test}" to proper style value.`, () => {
					...
					expect(getModelData(doc)).to.equal(
						`<paragraph>[]f<$text fontColor="${test.replace(
							/ /g,
							''
						)}">o</$text>o</paragraph>`
					);
					...
					expect(editor.getData()).to.equal(
						`<p>f<span style="color:${test.replace(
							/ /g,
							''
						)};">o</span>o</p>`
					);
		...
		it('should support <font color=".."> styling - #RRGGBB color', () => {
			const data =
				'<p><font color="#ACABAA">foo</font><span style="font-size: 18px;color: rgb(10, 20, 30);">bar</span>o</p>';
		...
		it('should support <font color=".."> styling - #RGB color', () => {
			const data =
				'<p><font color="#FAF">foo</font><span style="font-size: 18px;color: rgb(10, 20, 30);">bar</span>o</p>';
		...
		it('should support <font color=".."> styling - named color', () => {
			const data =
				'<p><font color="lightgreen">foo</font><span style="font-size: 18px;color: rgb(10, 20, 30);">bar</span>o</p>';
		...
		it('should support <font color=".."> styling - strip styling when color is incorrect', () => {
			const data =
				'<p><font color="rgb(11, 22, 33)">foo</font><span style="font-size: 18px;color: rgb(10, 20, 30);">bar</span>o</p>';
		...
		describe('nested elements that will be converted into the fontColor attribute', () => {
			it('should use the most inner value while converting nested font elements', () => {
				const data =
					'<p><font color="#00ffff"><font color="#ff0000">foo</font></font></p>';
			...
			it('should use the most inner value while converting a span with defined the color styles', () => {
				const data =
					'<p><font color="#00ffff"><span style="color:#ff0000">foo</span></font></p>';
			...
			it('should use the most inner value while converting the font element inside a span', () => {
				const data =
					'<p><span style="color:#ff0000"><font color="#00ffff">foo</font></span></p>';
			...
			it('should use the most inner value while converting nested spans with the color styles', () => {
				const data =
					'<p><span style="color:#00ffff"><span style="color:#ff0000">foo</span></span></p>';
