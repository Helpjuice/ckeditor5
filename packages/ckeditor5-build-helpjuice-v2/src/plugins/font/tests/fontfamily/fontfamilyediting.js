...
describe('FontFamilyEditing', () => {
	...
	it('should set proper schema rules', () => {
		...
		expect(editor.model.schema.checkAttribute(['$block'], 'fontFamily')).to
			.be.false;
	});

	it('should be marked with a formatting property', () => {
		expect(
			editor.model.schema.getAttributeProperties('fontFamily')
		).to.include({
			isFormatting: true,
		});
	});
	...
	describe('config', () => {
		describe('default value', () => {
			it('should be set', () => {
				...
				expect(
					editor.config.get('fontFamily.supportAllValues')
				).to.equal(false);
			});
		});
		...
		describe('supportAllValues=true', () => {
			...
			describe('editing pipeline conversion', () => {
				it('should convert unknown fontFamily attribute values', () => {
					setModelData(
						doc,
						'<paragraph>f<$text fontFamily="foo-bar">o</$text>o</paragraph>'
					);

					expect(editor.getData()).to.equal(
						'<p>f<span style="font-family:foo-bar;">o</span>o</p>'
					);
				});
				...
				it('should convert defined fontFamily attribute values', () => {
					setModelData(
						doc,
						'<paragraph>f<$text fontFamily="Arial, Helvetica, sans-serif">o</$text>o</paragraph>'
					);

					expect(editor.getData()).to.equal(
						'<p>f<span style="font-family:Arial, Helvetica, sans-serif;">o</span>o</p>'
					);
				});
			});
			...
			describe('data pipeline conversions', () => {
				it('should convert from an element with defined style when with other styles', () => {
					...
					expect(getModelData(doc)).to.equal(
						'<paragraph>[]f<$text fontFamily="Other">o</$text>o</paragraph>'
					);

					expect(editor.getData()).to.equal(
						'<p>f<span style="font-family:Other;">o</span>o</p>'
					);
				});
				...
				it('should convert from a complex definition', () => {
					...
					expect(getModelData(doc)).to.equal(
						'<paragraph>[]f<$text fontFamily="Arial,sans-serif">o</$text>o</paragraph>'
					);
				...
				it('should convert from a nested element', () => {
					const data =
						'<p>f<span><span><span><span style="font-family: Arial, sans-serif">o</span></span></span></span>o</p>';
				...
				it('should support <font face=".."> styling', () => {
					const data =
						'<p><font face="Arial,Verdana">foo</font><span style="font-family: Arial, sans-serif">bar</span></p>';
	...
	describe('editing pipeline conversion', () => {
		...
		it('should convert fontFamily attribute to configured simple preset', () => {
			setModelData(
				doc,
				'<paragraph>f<$text fontFamily="Arial">o</$text>o</paragraph>'
			);
		...
		it('should convert fontFamily attribute to configured complex preset', () => {
			...
			setModelData(
				doc,
				`<paragraph>f<$text fontFamily="${fontFamily}">o</$text>o</paragraph>`
			);

			expect(editor.getData()).to.equal(
				`<p>f<span style="font-family:${fontFamily};">o</span>o</p>`
			);
		});
		...
		it('should convert fontFamily attribute from user defined settings', () => {
			setModelData(
				doc,
				'<paragraph>f<$text fontFamily="my">o</$text>o</paragraph>'
			);

			expect(editor.getData()).to.equal(
				'<p>f<mark class="my-style">o</mark>o</p>'
			);
		});
	});
	...
	describe('data pipeline conversions', () => {
		...
		it('should convert from element with defined style when with other styles', () => {
			...
			expect(getModelData(doc)).to.equal(
				'<paragraph>[]f<$text fontFamily="my-other">o</$text>o</paragraph>'
			);
		...
		it('should convert from user defined element', () => {
			const data =
				'<p>f<mark class="my-style" style="font-family:Verdana;">o</mark>o</p>';
		...
		it('should strip <font face=".."> styling when supportAllValues = false', () => {
			const data =
				'<p><font face="Arial,Verdana">foo</font><span style="font-family: Arial">bar</span></p>';
