...
describe('FontBackgroundColorEditing', () => {
	...
	it('should have pluginName', () => {
		expect(FontBackgroundColorEditing.pluginName).to.equal(
	...
	it('has the attribute marked with the isFormatting property', () => {
		expect(
			editor.model.schema.getAttributeProperties('fontBackgroundColor')
		).to.include({
			isFormatting: true,
		});
	});
	...
	describe('config', () => {
		describe('default value', () => {
			it('should be set', () => {
				expect(
					editor.config.get('fontBackgroundColor.colors')
				).to.deep.equal([
					{
						color: 'hsl(0, 0%, 0%)',
						label: 'Black',
				...
				expect(
					editor.config.get('fontBackgroundColor.columns')
				).to.equal(5);
			});
		});
	});
		...
		describe('convert different color version', () => {
			...
			tests.forEach((test) => {
				it(`should convert fontBackgroundColor attribute: "${test}" to proper style value.`, () => {
					setModelData(
						doc,
						`<paragraph>fo<$text fontBackgroundColor="${test}">o b</$text>ar</paragraph>`
					);

					expect(editor.getData()).to.equal(
						`<p>fo<span style="background-color:${test};">o b</span>ar</p>`
					);
				});
			});
		});
	...
	describe('data pipeline conversions', () => {
		...
		it('should convert from element with defined style when with other styles (using background-color style)', () => {
			const data =
				'<p>f<span style="font-size: 18px;background: rgb(10, 20, 30);">o</span>o</p>';

			editor.setData(data);

			expect(getModelData(doc)).to.equal(
				'<paragraph>[]f<$text fontBackgroundColor="rgb(10,20,30)">o</$text>o</paragraph>'
			);

			expect(editor.getData()).to.equal(
				'<p>f<span style="background-color:rgb(10,20,30);">o</span>o</p>'
			);
		});

		it('should convert from element with defined style when with other styles (using background style)', () => {
			const data =
				'<p>f<span style="font-size: 18px;background-color: rgb(10, 20, 30);">o</span>o</p>';
		...
		describe('should convert from different color versions', () => {
			...
			tests.forEach((test) => {
				it(`should convert fontBackgroundColor attribute: "${test}" to proper style value.`, () => {
					...
					expect(getModelData(doc)).to.equal(
						`<paragraph>[]f<$text fontBackgroundColor="${test.replace(
							/ /g,
							''
						)}">o</$text>o</paragraph>`
					);
					...
					expect(editor.getData()).to.equal(
						`<p>f<span style="background-color:${test.replace(
							/ /g,
							''
						)};">o</span>o</p>`
					);
