...
describe('FontSizeEditing', () => {
	...
	it('should set proper schema rules', () => {
		expect(
			editor.model.schema.checkAttribute(['$block', '$text'], 'fontSize')
		).to.be.true;
		expect(
			editor.model.schema.checkAttribute(
				['$clipboardHolder', '$text'],
				'fontSize'
			)
		).to.be.true;

		expect(editor.model.schema.checkAttribute(['$block'], 'fontSize')).to.be
			.false;
	});

	it('should be marked with a formatting property', () => {
		expect(
			editor.model.schema.getAttributeProperties('fontSize')
		).to.include({
			isFormatting: true,
		});
	});
	...
	describe('config', () => {
		describe('default value', () => {
			it('should be set', () => {
				expect(editor.config.get('fontSize.options')).to.deep.equal([
					'tiny',
					'small',
					'default',
					'big',
				...
				expect(editor.config.get('fontSize.supportAllValues')).to.equal(
					false
				);
			});
		});
		...
		describe('supportAllValues=true', () => {
			...
			describe('editing pipeline conversion', () => {
				it('should pass pixel fontSize to data', () => {
					setModelData(
						doc,
						'<paragraph>f<$text fontSize="10px">o</$text>o</paragraph>'
					);

					expect(editor.getData()).to.equal(
						'<p>f<span style="font-size:10px;">o</span>o</p>'
					);
				});
				...
				it('should pass number fontSize to data', () => {
					setModelData(
						doc,
						'<paragraph>f<$text fontSize="12">o</$text>o</paragraph>'
					);

					expect(editor.getData()).to.equal(
						'<p>f<span style="font-size:12;">o</span>o</p>'
					);
				});
				...
				it('should pass percentage fontSize to data', () => {
					setModelData(
						doc,
						'<paragraph>f<$text fontSize="6.25%">o</$text>o</paragraph>'
					);

					expect(editor.getData()).to.equal(
						'<p>f<span style="font-size:6.25%;">o</span>o</p>'
					);
				});
				...
				it('should pass em fontSize to data', () => {
					setModelData(
						doc,
						'<paragraph>f<$text fontSize="8em">o</$text>o</paragraph>'
					);

					expect(editor.getData()).to.equal(
						'<p>f<span style="font-size:8em;">o</span>o</p>'
					);
				});
			});
			...
			describe('data pipeline conversions', () => {
				it('should convert from an element with defined style when with other styles', () => {
					const data =
						'<p>f<span style="font-family: Other;font-size: 18px">o</span>o</p>';

					editor.setData(data);

					expect(getModelData(doc)).to.equal(
						'<paragraph>[]f<$text fontSize="18px">o</$text>o</paragraph>'
					);

					expect(editor.getData()).to.equal(
						'<p>f<span style="font-size:18px;">o</span>o</p>'
					);
				});

				it('should convert from a nested element', () => {
					const data =
						'<p>f<span><span><span><span style="font-size: 18px">o</span></span></span></span>o</p>';
				...
				it('should convert <font size=".."> styling', () => {
					const data =
						'<font size="5">foo</font><span style="font-size: 18px">bar</span>';
				...
				it('should convert <font size=".."> styling - size = 0', () => {
					const data =
						'<font size="0">foo</font><span style="font-size: 18px">bar</span>';
				...
				it('should convert <font size=".."> styling - large value clamped to size 7 equivalent', () => {
					const data =
						'<font size="999">foo</font><span style="font-size: 18px">bar</span>';
				...
				it('should convert <font size=".."> styling - positive relative size', () => {
					const data =
						'<font size="+3">foo</font><span style="font-size: 18px">bar</span>';
				...
				it('should convert <font size=".."> styling - negative relative size', () => {
					const data =
						'<font size="-1">foo</font><span style="font-size: 18px">bar</span>';
				...
				it('should not convert <font size=".."> styling - parameter outside of the range', () => {
					const data =
						'<font size="1000">foo</font><span style="font-size: 18px">bar</span>';
	...
	describe('editing pipeline conversion', () => {
		...
		it('should discard unknown fontSize attribute values', () => {
			setModelData(
				doc,
				'<paragraph>f<$text fontSize="foo-bar">o</$text>o</paragraph>'
			);
		...
		it('should convert fontSize attribute to predefined named preset', () => {
			setModelData(
				doc,
				'<paragraph>f<$text fontSize="tiny">o</$text>o</paragraph>'
			);

			expect(editor.getData()).to.equal(
				'<p>f<span class="text-tiny">o</span>o</p>'
			);
		});
		...
		it('should convert fontSize attribute from user defined settings', () => {
			setModelData(
				doc,
				'<paragraph>f<$text fontSize="my">o</$text>o</paragraph>'
			);

			expect(editor.getData()).to.equal(
				'<p>f<mark class="my-style" style="font-size:30px;">o</mark>o</p>'
			);
		});
	});
	...
	describe('data pipeline conversions', () => {
					...
					options: [
						...
						},
						{
							...
							upcastAlso: [
								{
									name: 'span',
									styles: { 'font-size': '77em' },
								},
								{
									name: 'span',
									attributes: { 'data-size': '77em' },
								},
							],
						},
					],
		...
		it('should convert from element with defined multiple classes', () => {
			...
			expect(getModelData(doc)).to.equal(
				'<paragraph>[]f<$text fontSize="big-multiple">o</$text>o</paragraph>'
			);

			expect(editor.getData()).to.equal(
				'<p>f<span class="foo foo-big">o</span>o</p>'
			);
		});
		...
		it('should convert from element with defined style when with other styles', () => {
			const data =
				'<p>f<span style="font-family: serif;font-size: 18px">o</span>o</p>';
