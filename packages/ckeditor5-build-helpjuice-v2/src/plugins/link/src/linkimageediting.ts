...
export default class LinkImageEditing extends Plugin {
	...
	private _enableAutomaticDecorators(): void {
		...
		if (automaticDecorators.length) {
			editor.conversion
				.for('downcast')
				.add(automaticDecorators.getDispatcherForLinkedImage());
		}
	}
	...
	private _enableManualDecorators(): void {
		...
		for (const decorator of command.manualDecorators) {
			...
			editor.conversion
				.for('downcast')
				.add(downcastImageLinkManualDecorator(decorator));
			editor.conversion
				.for('upcast')
				.add(upcastImageLinkManualDecorator(editor, decorator));
		}
	}
}
		...
		dispatcher.on<UpcastElementEvent>(
			...
			(evt, data, conversionApi) => {
				...
				const blockImageView = imageInLink.findAncestor((element) =>
					imageUtils.isBlockImageView(element)
				);
	...
	return (dispatcher) => {
		dispatcher.on<DowncastAttributeEvent<Element>>(
			...
			(evt, data, conversionApi) => {
				...
				const viewFigure = conversionApi.mapper.toViewElement(
					data.item
				)!;
				const writer = conversionApi.writer;

				// But we need to check whether the link element exists.
				const linkInImage = Array.from(viewFigure.getChildren()).find(
					(child): child is ViewElement => child.is('element', 'a')
				);
				const viewImage = imageUtils.findViewImgElement(viewFigure)!;
				// <picture>...<img/></picture> or <img/>
				const viewImgOrPicture = viewImage.parent!.is(
					'element',
					'picture'
				)
					? viewImage.parent
				...
				if (linkInImage) {
					if (data.attributeNewValue) {
						...
						writer.move(
							writer.createRangeOn(viewImgOrPicture),
...
function downcastImageLinkManualDecorator(
	decorator: ManualDecorator
): (dispatcher: DowncastDispatcher) => void {
	return (dispatcher) => {
		dispatcher.on<DowncastAttributeEvent<Element>>(
			`attribute:${decorator.id}:imageBlock`,
			(evt, data, conversionApi) => {
				const viewFigure = conversionApi.mapper.toViewElement(
					data.item
				)!;
	...
	return (dispatcher) => {
		dispatcher.on<UpcastElementEvent>(
			...
			(evt, data, conversionApi) => {
				...
				const modelElement =
					(data.modelCursor.nodeBefore as Element) ||
					data.modelCursor.parent;
